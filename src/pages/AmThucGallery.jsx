...
import amThucData from "../data/amthuc.json";

const FALLBACK_IMAGE =
  "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=1200&q=80";

export default function AmThucGallery() {
  const { t } = useTranslation();
  const [selectedItem, setSelectedItem] = useState(null);

  const items = [...amThucData].sort((a, b) => a.so_thu_tu - b.so_thu_tu);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item) => (
          <button
            ...
            className="group text-left cursor-pointer bg-white dark:bg-gray-900 rounded-lg shadow hover:shadow-lg transition overflow-hidden border border-gray-200 dark:border-gray-700"
          >
            <div className="relative h-48 overflow-hidden bg-gray-100 dark:bg-gray-800">
              <img
                src={item.link_anh_mon_an || FALLBACK_IMAGE}
                alt={item.ten_mon_an}
            ...
            <div className="p-4">
              ...
              <div className="mt-4 inline-flex items-center text-sm font-medium text-[#0077D4] dark:text-blue-400">
      ...
      {selectedItem && (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="am-thuc-detail-title"
          ...
          >
            ...
            <img
              src={selectedItem.link_anh_mon_an || FALLBACK_IMAGE}
              alt={selectedItem.ten_mon_an}
              onError={handleImageError}
            ...
            <div className="p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#0077D4] dark:text-blue-400 mb-2">
                {t("amthuc.detailTitle")}
              </p>
              <h2
                id="am-thuc-detail-title"
                className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4 pr-10"
              >
                {selectedItem.ten_mon_an}
              </h2>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line">
                {selectedItem.mo_ta_mon_an}
              </p>
            </div>
          </div>
        </div>
